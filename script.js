...
// new initializeAuth that asks server
async function initializeAuth() {
    try {
        const resp = await fetch('auth_check.php');
...
async function login(email, password) {
    ...
    try {
        ...
        if (data.status === 'success') {
            ...
            // any page-specific updates
            if (window.location.pathname.includes('cart')) updateCartCount();
            if (window.location.pathname.includes('reservations')) checkUserLogin();
        } else {
...
async function register(email, password, name) {
    ...
    try {
        ...
        const resp = await fetch('register.php', { method: 'POST', body: form });
        const data = await resp.json();
        if (data.status === 'success') {
            currentUser = data.user;
            localStorage.setItem('currentUser', JSON.stringify(currentUser));
